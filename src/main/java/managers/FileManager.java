    ...
    public LinkedList<SpaceMarine> readCollection() {
        if(!fileName.isEmpty()) {
            ...
            try(FileInputStream fis = new FileInputStream(file)) {
                ...
                while (scanner.hasNextLine()){
                    ...
                    if(lineArr.length == 11) {
                        try {
                            ...
                            Console.printError("Can't parse data in line " + linenum + "!");
                            trashCounter++;
                        } catch (NotUniqueIdException e) {
                            Console.printError("Id not unique in line " + linenum + "!");
                        }
                    } else {
                        trashCounter++;
                        Console.printError("Can't parse data in line " + linenum + "!");
                ...
                Console.println("Successfully loaded " + result.size() + " Space Marines.");
                return result;
    ...
    public void saveCollection(LinkedList<SpaceMarine> spaceMarines) {
        if(!fileName.isEmpty()) {
            ...
            try(FileOutputStream fos = new FileOutputStream(file)) {
                ...
                Console.println("Collection saved successfully!");
